...
class Worker(queue: TaskQueue, config: PresentationCompilerConfig, reportContext: ReportContext):
  ...
  def cancelCurrentCancellable(newTaskKind: LspRequest): Unit = // TODO rename to TaskKind
    _currentTask.get() match
      case Some(t: queue.Cancellable[?]) if t.inputs.taskName == newTaskKind => t.interrupt()
      case _ =>
